...
test_data <- X_2016[,c("topic","stdv","st","rt","mean","median","min","max")]
...
tuneResult <- tune(svm, Y ~ X+Z+A+B+C,  data = train_data,
...
tuneResult <- tune(svm, Y ~ X+Z+A+B+C,  data = train_data,
                   ranges = list(epsilon = seq(0.35,0.45,0.01), cost = 2^(2:5))
) 
...
svrPredictionRMSE
# perform a grid search
tuneResult <- tune(svm, YYRT ~ X+Z+A+B+C,  data = train_data,
                   ranges = list(epsilon = seq(0,1,0.1), cost = 2^(2:3))
)
print(tuneResult)

tuneResult <- tune(svm, YYRT ~ X+Z+A+B+C,  data = train_data,
                   ranges = list(epsilon = seq(0.55,0.65,0.01), cost = 2^(2:5))
) 
...
write.csv(test_data, file = "bm25predicted2016threshold.txt",row.names=FALSE)
